...
describe('AccountBulkActionsBar', () => {
  ...
  it.each([
    ['all accounts', 'admin.accounts.bulkActions.deleteAllAccounts'],
    ['group A', 'admin.accounts.bulkActions.deleteCurrentGroup']
  ])('shows scoped delete without selecting rows: %s', async (groupName, label) => {
    const wrapper = mount(AccountBulkActionsBar, {
      props: {
        selectedIds: [], totalResults: 2001, selectingAll: false,
        allResultsSelected: false, canDeleteGroup: true, groupName,
        deleteGroupLabel: label
      }
    })
    ...
    await button!.trigger('click')
    expect(wrapper.emitted('delete-group')).toHaveLength(1)
