...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	if reqStream {
		...
		responseAuditBody = result.responseAuditBody
	} else {
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	for documentScanner.Scan() {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			// Keep every SSE payload for response-side usage audit, including
			// terminal and failed events. The collector is bounded.
			auditCollector.AppendSSEData(trimmedData)
...
func (s *OpenAIGatewayService) handleNonStreamingResponsePassthrough(
	...
	return &openaiNonStreamingResultPassthrough{
		...
		responseAuditBody: append([]byte(nil), body...),
	}, nil
}
