...
func (s *adminServiceImpl) deleteAccountIDs(ctx context.Context, accountIDs []int64) *BulkUpdateAccountsResult {
	return deleteAccountIDsWith(ctx, accountIDs, s.DeleteAccount)
}

func deleteAccountIDsWith(ctx context.Context, accountIDs []int64, deleteAccount func(context.Context, int64) error) *BulkUpdateAccountsResult {
	result := &BulkUpdateAccountsResult{
...
func (s *adminServiceImpl) DeleteAccountsByGroup(ctx context.Context, groupID int64) (*BulkUpdateAccountsResult, error) {
	...
	// Restrict cascades to the resolved snapshot: a parent with a shadow outside
	// this scope is left intact rather than deleting another group's account.
	allowed := make(map[int64]bool, len(accountIDs))
	deleted := make(map[int64]bool, len(accountIDs))
	for _, id := range accountIDs {
		allowed[id] = true
	}
	deleteInScope := func(ctx context.Context, id int64) error {
		if deleted[id] {
			return nil
		}
		shadows, err := s.accountRepo.ListShadowsByParent(ctx, id)
		if err != nil {
			return fmt.Errorf("list spark shadows for scoped delete: %w", err)
		}
		for _, shadow := range shadows {
			if !allowed[shadow.ID] {
				return fmt.Errorf("spark shadow %d is outside the selected group; parent %d retained", shadow.ID, id)
			}
		}
		// Use this checked list, not DeleteAccount's second unscoped lookup.
		for _, shadow := range shadows {
			if deleted[shadow.ID] {
				continue
			}
			if err := s.accountRepo.Delete(ctx, shadow.ID); err != nil {
				return fmt.Errorf("delete in-scope spark shadow %d: %w", shadow.ID, err)
			}
			deleted[shadow.ID] = true
		}
		if err := s.accountRepo.Delete(ctx, id); err != nil {
			return err
		}
	...
	return deleteAccountIDsWith(ctx, accountIDs, deleteInScope), nil
}
