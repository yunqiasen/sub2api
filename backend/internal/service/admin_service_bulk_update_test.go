...
func TestAdminServiceDeleteAccountsByGroupDeletesAllWhenGroupIDZero(t *testing.T) {
	repo := &accountRepoStubForBulkUpdate{
		listData: []Account{
			{ID: 7},
			{ID: 11},
	...
	require.Equal(t, int64(0), repo.lastListFilters.groupID)
...
type scopedDeleteAccountRepo struct {
	accountRepoStubForBulkUpdate
	shadows map[int64][]*Account
}

func (s *scopedDeleteAccountRepo) ListShadowsByParent(_ context.Context, id int64) ([]*Account, error) {
	return s.shadows[id], nil
}

func TestAdminServiceDeleteAccountsByGroupProtectsOutOfScopeShadows(t *testing.T) {
	repo := &scopedDeleteAccountRepo{
		accountRepoStubForBulkUpdate: accountRepoStubForBulkUpdate{
			listData: []Account{{ID: 7}, {ID: 11}}, listResult: &pagination.PaginationResult{Total: 2},
		},
		shadows: map[int64][]*Account{7: {{ID: 99}}},
	}
	svc := &adminServiceImpl{accountRepo: repo, groupRepo: &groupRepoStubForAdmin{getByID: &Group{ID: 12}}}
	result, err := svc.DeleteAccountsByGroup(context.Background(), 12)
	require.NoError(t, err)
	require.Equal(t, []int64{11}, repo.deletedIDs)
	require.Equal(t, []int64{7}, result.FailedIDs)
	require.Contains(t, result.Results[0].Error, "outside")
}

func TestAdminServiceDeleteAccountsByGroupCountsInScopeShadowsOnce(t *testing.T) {
	for _, ids := range [][]Account{{{ID: 7}, {ID: 11}}, {{ID: 11}, {ID: 7}}} {
		repo := &scopedDeleteAccountRepo{
			accountRepoStubForBulkUpdate: accountRepoStubForBulkUpdate{listData: ids, listResult: &pagination.PaginationResult{Total: 2}},
			shadows:                      map[int64][]*Account{7: {{ID: 11}}},
		}
		svc := &adminServiceImpl{accountRepo: repo}
		result, err := svc.DeleteAccountsByGroup(context.Background(), 0)
		require.NoError(t, err)
