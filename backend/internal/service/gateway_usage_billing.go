...
type RecordUsageInput struct {
	...
	RequestPrompt      string             // 用户可见 prompt 摘要（用于后台审计）
	UsageAudit         UsageAuditPayload  // 结构化请求/响应审计字段
...
func (s *GatewayService) buildRecordUsageLog(
	...
	usageAudit := input.UsageAudit
	if usageAudit.RequestPrompt == "" && input.RequestPrompt != "" {
		usageAudit.RequestPrompt = input.RequestPrompt
	}
