...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	return usageLogInsertPrepared{
		...
		args: []any{
			...
			log.AccountID,
			requestIDArg,
			nullString(log.RequestPrompt),
			nullString(log.SystemPromptSummary),
			nullString(log.SystemPromptText),
			nullString(log.DeveloperPromptText),
			nullStringSliceJSON(log.ToolNames),
			nullStringSliceJSON(log.ToolCallNames),
			nullToolCallAuditSliceJSON(log.ToolCallsJSON),
			nullString(log.OutputSummary),
			nullString(log.OutputText),
			nullString(log.RequestBodySHA256),
			nullInt(log.RequestBodyBytes),
			log.RequestBodyTruncated,
			log.ResponseTextTruncated,
			nullStringAnyMapJSON(log.TurnMetadata),
			nullStringAnyMapJSON(log.IPLocation),
			auditCaptureVersion,
			log.Model,
