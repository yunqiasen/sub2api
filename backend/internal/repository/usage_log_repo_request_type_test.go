...
func TestPrepareUsageLogInsert_PersistsAuditFields(t *testing.T) {
	...
	require.Len(t, prepared.args, len(usageLogInsertArgTypes))
	require.Equal(t, requestPrompt, requireOpsNullStringArg(t, prepared.args, 4).String)
	require.Equal(t, systemSummary, requireOpsNullStringArg(t, prepared.args, 5).String)
	require.Equal(t, systemText, requireOpsNullStringArg(t, prepared.args, 6).String)
	require.Equal(t, developerText, requireOpsNullStringArg(t, prepared.args, 7).String)
	require.JSONEq(t, `["exec_command"]`, requireOpsStringArg(t, prepared.args, 8))
	require.JSONEq(t, `["exec_command"]`, requireOpsStringArg(t, prepared.args, 9))
	require.Contains(t, requireOpsStringArg(t, prepared.args, 10), "exec_command")
	require.Equal(t, outputSummary, requireOpsNullStringArg(t, prepared.args, 11).String)
	require.Equal(t, outputText, requireOpsNullStringArg(t, prepared.args, 12).String)
	require.Equal(t, bodyHash, requireOpsNullStringArg(t, prepared.args, 13).String)
	require.Equal(t, int64(bodyBytes), requireOpsNullInt64Arg(t, prepared.args, 14).Int64)
	require.Equal(t, true, prepared.args[15])
	require.Equal(t, true, prepared.args[16])
	require.JSONEq(t, `{"session_id":"s1"}`, requireOpsStringArg(t, prepared.args, 17))
	require.JSONEq(t, `{"source":"direct"}`, requireOpsStringArg(t, prepared.args, 18))
