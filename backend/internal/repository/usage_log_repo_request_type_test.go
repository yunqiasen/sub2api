...
func TestBuildUsageLogBestEffortInsertQuery_IncludesRequestedModelColumn(t *testing.T) {
	...
	require.Equal(t, prepared.args[21], args[21])
}
...
func TestPrepareUsageLogInsert_PersistsNativeCompactionV2WithoutChangingRequestType(t *testing.T) {
	...
	require.Equal(t, int16(service.RequestTypeStream), prepared.args[46])
...
func TestPrepareUsageLogInsert_PersistsImageSizeMetadata(t *testing.T) {
	...
	require.Equal(t, sql.NullString{String: imageSize, Valid: true}, prepared.args[54])
	require.Equal(t, sql.NullString{String: inputSize, Valid: true}, prepared.args[55])
	require.Equal(t, sql.NullString{String: outputSize, Valid: true}, prepared.args[56])
	require.Equal(t, sql.NullString{String: source, Valid: true}, prepared.args[57])
	breakdownJSON, ok := prepared.args[58].(string)
	require.True(t, ok)
...
func TestScanUsageLogRequestTypeAndLegacyFallback(t *testing.T) {
	t.Run("image_size_metadata_is_scanned", func(t *testing.T) {
		...
		log, err := scanUsageLog(usageLogScannerStub{values: []any{
			...
			sql.NullString{}, // request_prompt
			sql.NullString{}, // system_prompt_summary
			sql.NullString{}, // system_prompt_text
			sql.NullString{}, // developer_prompt_text
			sql.NullString{}, // tool_names
			sql.NullString{}, // tool_call_names
			sql.NullString{}, // tool_calls_json
			sql.NullString{}, // output_summary
			sql.NullString{}, // output_text
			sql.NullString{}, // request_body_sha256
			sql.NullInt64{},  // request_body_bytes
			false,            // request_body_truncated
			false,            // response_text_truncated
			sql.NullString{}, // turn_metadata
			sql.NullString{}, // ip_location
			int16(1),         // audit_capture_version
			"gpt-image-2",
