	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			rows := sqlmock.NewRows([]string{
				...
				"status_code", "client_ip", "request_prompt", "error_id", "phase", "severity", "message", "user_id", "api_key_id", "account_id", "group_id", "stream",
			}).
				AddRow("success", start, "req-slow", "openai", "gpt-5.5", 12000, 800, nil, "192.0.2.1", "audit prompt", nil, nil, nil, nil, 1, 2, 3, 4, true).
				AddRow("error", start, "req-zero", "openai", "gpt-5.5", 9000, 0, 502, nil, nil, 5, "upstream", "error", "failed", 1, 2, 3, 4, true).
				AddRow("success", start, "req-missing", "openai", "gpt-5.5", 5000, nil, nil, nil, nil, nil, nil, nil, nil, 1, 2, 3, 4, false)
			...
			require.Equal(t, "192.0.2.1", *items[0].ClientIP)
			require.Equal(t, "audit prompt", *items[0].RequestPrompt)
