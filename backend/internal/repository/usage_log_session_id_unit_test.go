...
func TestPrepareUsageLogInsert_SessionIDArgWiring(t *testing.T) {
	require.Len(t, usageLogInsertArgTypes, 78, "arg-type table must include session_id")
...
func TestPrepareUsageLogInsert_RequestedReasoningEffortArgWiring(t *testing.T) {
	...
	require.Len(t, prepared.args, len(usageLogInsertArgTypes))
	require.Equal(t, "text", usageLogInsertArgTypes[64], "requested_reasoning_effort must follow reasoning_effort")
	require.Equal(t, "text", usageLogInsertArgTypes[63], "reasoning_effort arg type must stay text")

	forwardedArg, ok := prepared.args[63].(sql.NullString)
	require.True(t, ok)
	require.True(t, forwardedArg.Valid)
	require.Equal(t, forwarded, forwardedArg.String)

	requestedArg, ok := prepared.args[64].(sql.NullString)
	require.True(t, ok)
